...
class BaranggayAdminController extends Controller
{
    public function index(ChartController $chart, Request $request)
    {
        $currentDate = Carbon::now()->format('Y-m-d');
        $assignedBaranggay = Auth::user()->baranggay->baranggay_name;
        $currentDate = Carbon::now()->format('Y-m-d');
        ...
        $persons = PersonWithDisability::where('present_baranggay', '=', $assignedBaranggay)->simplePaginate(5);
        $personDisability = PersonWithDisability::with('disabilityType')->leftJoin('disability_types', 'disability_types.id', '=', 'person_with_disabilities.id')->where('present_baranggay', '=', $assignedBaranggay)->get();
        $personQuery = PersonWithDisability::where('present_baranggay', '=', $assignedBaranggay);

        $grandMale = $persons->filter(fn($person) => $person->sex === 'Male')->count();
        $grandFemale = $persons->filter(fn($person) => $person->sex === 'Female')->count();

        $totalSingle = $persons->filter(fn($person) => $person->civil_status === 'Single')->count();
        $totalMarried = $persons->filter(fn($person) => $person->civil_status === 'Married')->count();
        $totalWidowed = $persons->filter(fn($person) => $person->civil_status === 'Widowed')->count();

        $sophomore = $persons->filter(fn($person) => Carbon::parse($person->date_of_birth)->age < 19)->count();
        $junior = $persons->filter(fn($person) => Carbon::parse($person->date_of_birth)->age > 19 && Carbon::parse($person->date_of_birth)->age < 65)->count();
        $senior = $persons->filter(fn($person) => Carbon::parse($person->date_of_birth)->age >= 65)->count();

        $active = $persons->filter(fn($person) => $person->submittedForm->status === 'Active')->count();
        $moved = $persons->filter(fn($person) => $person->submittedForm->status === 'Moved')->count();
        $deceased = $persons->filter(fn($person) => $person->submittedForm->status === 'Deceased')->count();
        ...
        return view('baranggay-admin.dashboard', [
            'date' => $currentDate,
            'persons' => $persons,
            'sexChart' => $chart->overAllGender($grandMale, $grandFemale),
            ...
            'disabilityTypeChart' => $chart->disabilityType($disabilityCounts),
            'sort' => $sort,
    ...
    public function createPWD()
    {
        ...
        return view('baranggay-admin.add-pwd', [
            'generateId' => $randomAccountId
        ]);
    }
    public function viewPWD(PersonWithDisability $personWithDisability)
    {
    ...
    public function storePWD(Request $request)
    {
        ...
        return redirect()->route('baranggay-dashboard')->with('message', 'PWD Added successfully.');
    }
    public function editPWDView(PersonWithDisability $personWithDisability)
    {
        ...
        return view('baranggay-admin.edit-pwd', [
            'person' => $personWithDisability,
    ...
    public function storeUpdatedPWD(Request $request, PersonWithDisability $personWithDisability, EmploymentRecord $employmentRecord)
    {
        ...
        return redirect()->route('baranggay-dashboard')->with('message', 'PWD Information updated successfully.');
    }
    public function eventsAndPrograms(Request $request)
    {
        ...
        return view('baranggay-admin.events-program.dashboard', [
            'events' => $events,
    ...
    {
        return view('baranggay-admin.events-program.create-event');
    }
    public function editEvent(Event $event)
    {
        ...
        return view('baranggay-admin.events-program.edit-event', [
            'event' => $event,
    ...
    public function storeUpdatedEvent(Request $request, Event $event)
    {
        ...
        return redirect('/baranggay-admin/events-programs')->with('message', 'Program updated successfully.');
    }
    ...
    public function storeEvent(Request $request)
    {
        ...
        return redirect('baranggay-admin/events-programs')->with('message', 'Event added successfully.');
    }
    ...
    public function viewProgram(Event $event, Request $request)
    {
        ...
        return view('baranggay-admin.events-program.view-program', [
            'program' => $event,
            'visualImpairment' => $visualImpairment,
            'learningDisabilities' => $learningDisabilities,
            'hearingImpairment' => $hearingImpairment,
    ...
    public function viewProgramReport(Event $event, ChartController $chart)
    {
        ...
        return view('baranggay-admin.events-program.program-report', [
    ...
    public function viewProgramInvitation(Request $request)
    {
        ...
        return view('baranggay-admin.events-program.invitation', [
    ...
    public function sendProgramInvitation(Request $request)
    {
        ...
        return redirect('baranggay-admin/events-programs/event/invitation/sent');
    }
    ...
    public function sendingInvitation()
    {
        ...
        return redirect('/baranggay-admin/events-programs')->with('message', 'Person invited successfully.');
    }
}
