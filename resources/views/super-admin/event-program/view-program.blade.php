        ...
        <section class="w-[60%]">
            <header class="w-full flex items-center justify-between bg-blue-800 p-3">
                ...
                <article class="flex flex-col gap-3">
                    <a href="/super-admin/dashboard/events-programs/{{ $program->id }}/program/edit"><img src="{{ asset('assets/pencil.png') }}" alt="Edit Image" class="w-7 bg-white p-[7px] rounded-lg"></a>
                    <a href="/program/pdf/{{ $program->id }}"><img src="{{ asset('assets/download.png') }}" alt="Download Image" class="w-7 bg-white p-[7px] rounded-lg"></a>
                </article>
            </header>

            <form action="/super-admin/dashboard/events-programs/{{ $program->id }}/program-report"  class="flex">
                @csrf
                <aside class="w-[50%] p-5 flex items-start gap-5 flex-col">
                    ...
                    <article class="flex items-center w-full justify-between gap-5 lg:flex-col xl:flex-row lg:items-stretch">
                        <div class="flex-1 flex items-center gap-x-3">
                            ...
                            <x-input-box class="input-box flex-1" type="date" value="{{ $program->date }}"/>
                        </div>
            
                        <div class="flex-1 flex items-center gap-x-3">
                            <label class=" font-bold">Time</label>
                            <x-input-box class="input-box flex-1" type="time" value="{{ $program->time }}"/>
                        </div>
                    </article>
        ...
        <section class="flex flex-col items-start flex-1 ">
            <header class="w-full flex flex-col bg-red-700 p-[10px]">
                ...
                <article class="flex items-center justify-between">
                    <form action="/super-admin/dashboard/events-programs/{{ $program->id }}/program" method="GET" class="relative p-2 w-fit">
                        @csrf
                    ...
                    <form action="/super-admin/dashboard/events-programs/{{ $program->id }}/program" method="GET">
                        @csrf
                        <select name="sort" class="normal-button" onchange="this.form.submit()">
                            <option selected disabled>Sort By</option>
                            <option value="id" @selected($sort === 'id')>ID</option>
                            <option value="name" @selected($sort === 'name')>Name</option>
                            <option value="baranggay" @selected($sort === 'baranggay')>Barangay</option>
                        </select>
                    </form>
                </article>
            </header>
            ...
            <aside class="w-full p-5">
                ...
                <article class="flex items-center justify-center mb-5">
                    <form action="{{ route('program-invitation') }}" method="POST" class="w-full">
                        @csrf
                ...
                <form action="/super-admin/dashboard/events-programs/attendance/add" method="POST" id="attendanceForm">
                    @csrf
                    <table class="border-collapse border border-gray-400 rounded-lg w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="text-sm font-bold">Phone Number</th>
                            ...
                            @foreach ($invitedPerson as $person)
                                ...
                                @if (count($alignedDisabilities) > 0)
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-xs text-center">{{ $person->personWithDisability->first_name}} {{$person->personWithDisability->last_name}}</td>
                                        <td class="text-xs text-center">{{ $person->personWithDisability->present_baranggay }}</td>
                                        <td class="text-xs text-center"><a href="/super-admin/dashboard/events-programs/{{ $person->personWithDisability->id }}/contact/{{ $program->id }}">{{ $person->personWithDisability->contact_no }}</a></td>
