<x-dashboard-layout>
    ...
    <section class="flex items-center justify-center w-full h-screen">
        <form action="/super-admin/birhtday-cash-gift/update-status/{{ $person->id }}" method="POST" class="border-gray-300 border-2 p-5 rounded-lg bg-white shadow-lg flex flex-col gap-5" enctype="multipart/form-data">
            @if ($errors->any())
            ...
            <div class="flex flex-col w-full gap-2">
                ...
                <a href="{{ route('cashGifts.barangay.super-admin', $barangay->id ) }}" class="w-full p-2 rounded-lg text-xs bg-red-700 text-center text-white">Back</a>
            </div>
        </form>
    </section>
</x-dashboard-layout>